...
public class HbaseRepositoryFactoryBean<R extends HbaseRepository<T, ID>, T, ID> implements FactoryBean<R> {
    ...
    public HbaseRepositoryFactoryBean(HbaseRepositoryInfo<T, R, ID> repositoryInfo) {
        Optional.ofNullable(repositoryInfo).map(HbaseRepositoryInfo::getHbaseEntity)
        ...
        this.repositorySupplier = () -> getRepository(repositoryInterface, repositoryInfo.getHbaseEntity());
    }
    
    private R getRepository(Class<R> repositoryInterface, HbaseEntity<T, ID> hbaseEntity) {
        Optional.ofNullable(hbaseEntity).orElseThrow(() -> new ParseException("HbaseEntity"));
        ProxyFactory proxyFactory = new ProxyFactory();
        SimpleHbaseRepository<T, ID> defaultHbaseCrudRepository = new SimpleHbaseRepository<>(hbaseEntity);
